class EstimatesController < ApplicationController
  ...
  def email_estimate
    ...
    begin
      @estimate = Estimate.create(job_id: params[:id], job_type_id: JobType.find(:first, conditions: {kind: params[:type].capitalize}).id, flashvars: @job.options_for_print(params[:type].capitalize).with_indifferent_access, token: SecureRandom.hex(16))
