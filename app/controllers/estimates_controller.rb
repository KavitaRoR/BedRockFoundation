class EstimatesController < ApplicationController
  ...
  def email_estimate
    ...
    begin
      if @job.job_calc_type == "adhoc" || @job.foundation_kind.downcase.include?("concrete")
        ...
        if !@estimate
          @estimate = Estimate.create(job_id: params[:job_id], job_type_id: @job_type.id, flashvars: @job.options_for_print(params[:type].capitalize).with_indifferent_access)
        end
      end
  ...
  def view_estimate
    ...
    @options_for_job = YAML::load(@estimate.flashvars).with_indifferent_access
    @type = @estimate.job_type.kind
    session[:return_to] = "/e/#{params[:token]}?type=#{@type}"
    wepay_vars
  end
  ...
  def push_to_sold
    ...
    begin
      ...
      if !@estimate
        @estimate = Estimate.create(job_id: params[:id], job_type_id: @job_type.id, flashvars: @job.options_for_print(params[:type].capitalize).with_indifferent_access)
      end
