class EstimateMailer < ActionMailer::Base
  ...
  def send_to_contact(estimate)
    ...
    mail :to => "#{@estimate.job.contact.first_name + " " + @estimate.job.contact.last_name } <#{@estimate.job.contact.email}>", :from => "stoltzfus@abundantscapes.com", :subject => "Estimate from Bed Rock Foundations"
  end
end
