...
AngularFire.prototype = {
  ...
  associate: function($scope, name, type) {
    ...
    this._fRef.on("value", function(snap) {
      ...
      self._log(name+ "--------")
      // self._log(self._remoteValue)
      // self._log(typeof self._remoteValue)
      if (self._remoteValue.constructor.name === "Array") {
        var data = [];
        angular.forEach(self._remoteValue, function(item){
        ...
        var data = self._remoteValue
      }
      self._log(data.constructor.name)
      // Update the local model to reflect remote changes.
      self._timeout(function() {
        self._resolve($scope, name, resolve, data)
      });
    });
    return promise;
  },
